...
import { validateEmail, isPlatformIOS, getCodeFromText } from '../../utility/Utils'
...
class OtpScreen extends Component {
    ...
    validateCode = () => {
        ...
        if ( code === "" || code === " " || code === null || code.length <= 4) {
        ...
        if (this.validateCode()) {
            ...
            if (validateEmail(this.data)) {
                this.props.hitVerifyEmailWithOtpApi(this.data, code)
                    .then(
                        (data) => {
                            this.onVerifyCode_Success(data);
                        }
                    ).catch((error) => {
    ...
    onVerifyCode_Success = (data) => {
        if (data.status_code == 200) {
            this.props.onVerifyOtpSuccessAction(data)
            AppToast(data.message);
            if (this.callback) {
                this.callback && this.callback(this.state.code)
                this.navigate()
            ...
            <View style={styles.fullScreen}>
                ...
                <View style={styles.parentView}>
                    ...
                    <View style={styles.buttonContainer}>
                        <CustomButton rounded btnText={strings.VERIFY} onPress={this.submitCodeHandler} />
                    </View>
