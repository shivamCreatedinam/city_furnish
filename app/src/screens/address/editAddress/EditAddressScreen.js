...
import { View, Text, Image, TouchableOpacity, Keyboard, PermissionsAndroid, Platform } from 'react-native'
...
import { validateMobileNumber, renderInputError, focusTo, validatePostal, isPlatformIOS, MapAPIKey } from '../../../utility/Utils'
...
class EditAddressScreen extends Component {
    ...
    fetchNonServiceablePostalCode = () => {
        this.props.getNonServiceablePostalCode()
            .then((data) => {
    ...
    getCurrentAddress = () => {
            fetch("https://maps.googleapis.com/maps/api/geocode/json?address=" + this.state.region.latitude+"," +this.state.region.longitude +"&key=" + MapAPIKey).then((response) => response.json()).then((responseJson) => {
    ...
    setAddressFieldAutoPopulate = (responseJson) => {
        ...
        var pos = address.lastIndexOf(','), withoutComma = "";
        ...
        console.log(country + " == " + state + " == " + city + " == " + postcode + " == " + address)

        // check city Validation
        city= this.cityValidationCondition(city);

        if(!this.cityExists(city) && this.state.city !== city) {
        ...
        if(this.state.listViewDisplayed && this.state.navigationFlag) {
            this.setAddressData()
        }
    }
    ...
    cityValidationCondition= (city) => {
        if(city.includes('Bangalore') || city.includes('Bengaluru') ) {
            ...
            return 'Mumbai'
        } else if(city.includes('Gautam Buddh Nagar') || city.includes('Greater Noida') ) {
            return 'Noida'
    ...
    render() {
        const { fullName, mobileNumber, alternateMobileNumber, addressLine1, addressLine2, city, state, postalCode, error, isDefault } = this.state
        const { item } = this.itemList
        return (
                        ...
                        <View style={styles.mapBox}>
                            <React.Fragment>
                                <View style={styles.panelUpperOuter}>
                                    <View style={[styles.panelHeaderOuter,
                                    this.state.listViewDisplayed? styles.panelFillOuter:styles.panelOuter]}>
                                        <GooglePlacesAutocomplete
                                            ...
                                            renderLeftButton={()=>
                                                (<View style={styles.searchIconOuter}>
                                                    <Icon name="search" size={20} color="#DC2B6B" type="ionicon" />
                                                </View>)
                                            }
                                            textInputProps={{
                                                onChangeText: (text) => {
                                    ...
                                    {/* <View style={[styles.panelORHeaderOuter]}>
                                        <Text style={styles.mapORTextOuter}>
                                            {' OR '}
                                        </Text>
                                    </View> */}
                                    <View style={[styles.panelBtnHeaderOuter]}>
                                        <TouchableOpacity 
                                            ...
                                            style={styles.panelMapButtonOuter}>
                                            <Text style={styles.mapBtnTextOuter}>
                                                <Icon name="map-marker" size={18} color={resources.colors.appColor} type="fontawesome" /> {' '} Use current location
                                            </Text>
                                        </TouchableOpacity>
                                    </View>
                                </View>
    ...
    addNewAddress = () => {
        ...
        if (!isValid) { return; }
        const { fullName, mobileNumber, alternateMobileNumber, addressLine1, addressLine2, city, country, postalCode, isDefault, shipId, state } = this.state

        this.props.hitAddAddressApi(fullName, addressLine1, addressLine2, city, state, country, mobileNumber, alternateMobileNumber, postalCode, +isDefault, shipId)
            .then((data) => {
    ...
    validate = () => {
        ...
        if (alternateMobileNumber == "" || alternateMobileNumber == undefined || alternateMobileNumber == null) {
            errorObject.alternateMobileerr = resources.strings.alternatePhoneCannotBeEmpty;
        ...
        else if (!validateMobileNumber(alternateMobileNumber)) {
            errorObject.alternateMobileerr = resources.strings.enterValidPhone;
        }
        if (addressLine1 == "") {
        ...
        } else if(!this.cityExists(city)) {
            errorObject.Cityerr = resources.strings.cityCannotBeDelivered
        }
        if (state == "") {
